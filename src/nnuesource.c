...
int main(int argc, char **argv) {
	...
	fprintf(g, "alignas(64) ft_weight_t ft_weights[K_HALF_DIMENSIONS * FT_IN_DIMS] = {");
	print_ft_weights(g, ft_weights, SIZE(ft_weights));
	fprintf(g, "\n};\n\n");
	fprintf(g, "alignas(64) ft_bias_t ft_biases[K_HALF_DIMENSIONS] = {");
	print_ft_biases(g, ft_biases, SIZE(ft_biases));
	fprintf(g, "\n};\n\n");

	fprintf(g, "alignas(64) ft_weight_t psqt_weights[1 * FT_IN_DIMS] = {");
	print_psqt_weights(g, ft_weights, SIZE(ft_weights));
	fprintf(g, "\n};\n\n");

	fprintf(g, "alignas(64) weight_t hidden1_weights[32 * FT_OUT_DIMS] = {");
	print_weights(g, hidden1_weights, SIZE(hidden1_weights));
	fprintf(g, "\n};\n\n");
	fprintf(g, "alignas(64) bias_t hidden1_biases[32] = {");
	print_biases(g, hidden1_biases, SIZE(hidden1_biases));
	fprintf(g, "\n};\n\n");

	fprintf(g, "alignas(64) weight_t hidden2_weights[32 * 32] = {");
	print_weights(g, hidden2_weights, SIZE(hidden2_weights));
	fprintf(g, "\n};\n\n");
	fprintf(g, "alignas(64) bias_t hidden2_biases[32] = {");
	print_biases(g, hidden2_biases, SIZE(hidden2_biases));
	fprintf(g, "\n};\n\n");

	fprintf(g, "alignas(64) weight_t output_weights[1 * 32] = {");
	print_weights(g, output_weights, SIZE(output_weights));
	fprintf(g, "\n};\n\n");
	fprintf(g, "alignas(64) bias_t output_biases[1] = {");
