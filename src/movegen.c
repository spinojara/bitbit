...
static inline move_t *movegen_pawn(const struct position *pos, const struct pstate *pstate, move_t *moves,
                                   uint64_t targets, unsigned type) {
	...
	if (type & MOVETYPE_NONQUIET) {
		 ...
		 */
		if (pos->en_passant
		    && (!pstate->checkers || shift(bitboard(pos->en_passant), down) == pstate->checkers))
	...
	if (pawns && type & MOVETYPE_QUIET) {
		...
		uint64_t push_twice   = pawns & shift_twice(push_targets, down) & (us ? RANK_2 : RANK_7)
		                    & ~shift(all, down);
...
static inline move_t *movegen_king(const struct position *pos, const struct pstate *pstate, move_t *moves,
                                   unsigned type) {
	const int us     = pos->turn;
	const int them   = other_color(us);
	uint64_t own     = pos->piece[us][ALL];
	...
	if (type & MOVETYPE_QUIET && !pstate->checkers) {
		if (us) {
			if (pos->castle & 0x1 && !(all & 0x60) && !(pstate->attacked[ALL] & 0x60))
				*moves++ = new_move(e1, g1, MOVE_CASTLE, 0);
			if (pos->castle & 0x2 && !(all & 0xE) && !(pstate->attacked[ALL] & 0xC))
