...
static const uint64_t polyglot_keys[781] = {
...
static inline int polyglot_piece(int piece) { return color_of_piece(piece) + 2 * (uncolored_piece(piece) - PAWN); }

static inline uint64_t polyglot_piece_key(int piece, int square) {
	return polyglot_keys[64 * polyglot_piece(piece) + square];
}

static inline uint64_t polyglot_castle_key(const struct position *pos) {
	uint64_t key = 0;
	for (int i = 0; i < 4; i++)
		if (pos->castle & (1 << i))
			key ^= polyglot_keys[768 + i];
	return key;
}

static inline uint64_t polyglot_en_passant_key(const struct position *pos) {
	if (!pos->en_passant)
		return 0;

	if (adjacent_files(pos->en_passant) & shift(rank(pos->en_passant), pos->turn ? S : N)
	    & pos->piece[pos->turn][PAWN])
		return polyglot_keys[772 + file_of(pos->en_passant)];

	return 0;
}

static inline uint64_t polyglot_turn_key(const struct position *pos) { return pos->turn ? polyglot_keys[780] : 0; }

static uint64_t polyglot_key(const struct position *pos) {
	uint64_t piece = 0;
	for (int i = 0; i < 64; i++)
		if (pos->mailbox[i])
			piece ^= polyglot_piece_key(pos->mailbox[i], i);

	return piece ^ polyglot_castle_key(pos) ^ polyglot_en_passant_key(pos) ^ polyglot_turn_key(pos);
}

static move_t polyglot_random_move(FILE *f, const struct position *pos, uint64_t *seed) {
	fseek(f, 0, SEEK_SET);
	uint64_t key            = 0;
	uint64_t pk             = polyglot_key(pos);
	int nmoves              = 0;
	move_t moves[MOVES_MAX] = { 0 };
	uint16_t weights[MOVES_MAX];
	uint32_t weight = 0;
	...
	while (key <= pk) {
		...
		if (fread(buf, 1, 16, f) != 16)
			break;
		key = (uint64_t)buf[7] | (uint64_t)buf[6] << 8 | (uint64_t)buf[5] << 16 | (uint64_t)buf[4] << 24
		...
		move_t move   = M((m >> 6) & 0x3F, m & 0x3F, promotion ? MOVE_PROMOTION : 0,
                                promotion ? promotion - 1 : 0);

		if (uncolored_piece(pos->mailbox[move_from(&move)]) == KING) {
			if (move_from(&move) == e1) {
				if (move_to(&move) == a1)
					move = M(e1, c1, MOVE_CASTLE, 0);
				else if (move_to(&move) == h1)
					move = M(e1, g1, MOVE_CASTLE, 0);
			}
			else if (move_from(&move) == e8) {
				if (move_to(&move) == a8)
					move = M(e8, c8, MOVE_CASTLE, 0);
				else if (move_to(&move) == h8)
					move = M(e8, g8, MOVE_CASTLE, 0);
			}
		}
		else if (pos->en_passant && uncolored_piece(pos->mailbox[move_from(&move)]) == PAWN
		         && (int)move_to(&move) == pos->en_passant)
			move = M(move_from(&move), move_to(&move), MOVE_EN_PASSANT, 0);
		...
		moves[nmoves]    = move;
		weights[nmoves]  = buf[11] | (buf[10] << 8);
		weight          += weights[nmoves];
	...
	if (weight) {
		uint32_t u = uniformint(seed, 0, weight);
