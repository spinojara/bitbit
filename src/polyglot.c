...
static inline int polyglot_piece(int piece) {
	return color_of_piece(piece) + 2 * (uncolored_piece(piece) - PAWN);
}
...
static inline uint64_t polyglot_en_passant_key(const struct position *pos) {
	...
	if (adjacent_files(pos->en_passant) & shift(rank(pos->en_passant), pos->turn ? S : N) & pos->piece[pos->turn][PAWN])
		return polyglot_keys[772 + file_of(pos->en_passant)];

	return 0;
}

static inline uint64_t polyglot_turn_key(const struct position *pos) {
	return pos->turn ? polyglot_keys[780] : 0;
}
...
static move_t polyglot_random_move(FILE *f, const struct position *pos, uint64_t *seed) {
	...
	while (key <= pk) {
		...
		else if (pos->en_passant && uncolored_piece(pos->mailbox[move_from(&move)]) == PAWN && (int)move_to(&move) == pos->en_passant)
