...
static inline int32_t push_toward(int square1, int square2) { return 7 - distance(square1, square2); }
...
int32_t evaluate_KRKP(const struct position *pos, int strong_side) {
	...
	if (p == BITBASE_WIN)
		eval += VALUE_WIN + material_value[ROOK] - material_value[PAWN] + push_toward(strong_king, weak_king)
		      - pos->halfmove;
	else if (p == BITBASE_LOSE)
...
int32_t evaluate_KRKN(const struct position *pos, int strong_side) {
	...
	return 2 * push_toward_edge(weak_king) + push_away(weak_king, weak_knight)
...
int32_t evaluate_KBNK(const struct position *pos, int strong_side) {
	...
	return VALUE_WIN + material_value[BISHOP] + material_value[KNIGHT] + push_toward(strong_king, weak_king)
...
int32_t evaluate_KQKX(const struct position *pos, int strong_side) {
	...
	return VALUE_WIN + material_value[QUEEN] - material_value[piece] + push_toward(strong_king, weak_king)
...
int32_t evaluate_KXK(const struct position *pos, int strong_side) {
	...
	return VALUE_WIN + material + 0x4 * pawn_bonus + push_toward(strong_king, weak_king)
	     + 0x8 * push_toward_edge(weak_king) - pos->halfmove;
}
