...
int see_geq(struct position *pos, const move_t *move, int32_t value) {
	...
	attackers |= bishop_attacks(to, 0, occupied) & (pos->piece[WHITE][BISHOP] | pos->piece[BLACK][BISHOP]);

	attackers |= rook_attacks(to, 0, occupied) & (pos->piece[WHITE][ROOK] | pos->piece[BLACK][ROOK]);

	attackers |= queen_attacks(to, 0, occupied) & (pos->piece[WHITE][QUEEN] | pos->piece[BLACK][QUEEN]);

	attackers |= king_attacks(to, 0) & (pos->piece[WHITE][KING] | pos->piece[BLACK][KING]);

	if (generate_checkers(pos, other_color(us)) & ~tob
	    && (!(attackers & pos->piece[other_color(us)][KING]) || attackers & pos->piece[us][ALL])) {
		if (victim) {
			pos->piece[them][victim] ^= tob;
			pos->piece[them][ALL]    ^= tob;
		}
	...
	uint64_t pinners[2]   = { generate_pinners(pos, pinned[BLACK], BLACK),
		                  generate_pinners(pos, pinned[WHITE], WHITE) };
	...
	while (1) {
		...
		if (turnattackers & pos->piece[turn][PAWN]) {
			...
			attackers |= bishop_attacks(to, 0, occupied)
			           & (pos->piece[WHITE][BISHOP] | pos->piece[WHITE][QUEEN] | pos->piece[BLACK][BISHOP]
			...
			attackers |= rook_attacks(to, 0, occupied)
			           & (pos->piece[WHITE][ROOK] | pos->piece[WHITE][QUEEN] | pos->piece[BLACK][ROOK]
			              | pos->piece[BLACK][QUEEN]);
		}
