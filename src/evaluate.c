...
void evaluate_print(struct position *pos) {
	...
	alignas(64) int32_t psqtaccumulation[2][8] = { 0 };
	...
	for (int r = 7; r >= 0; r--) {
		...
		for (int f = 0; f < 8; f++) {
			...
			if (uncolored_piece(piece) && uncolored_piece(piece) != KING) {
				int16_t oldeval = psqtaccumulation[WHITE][bucket] - psqtaccumulation[BLACK][bucket];
				for (int color = 0; color < 2; color++) {
					int king_square = ei.king_square[color];
					king_square = orient(color, king_square, king_square);
					int index = make_index(color, square, piece, king_square);
					add_index_slow(index, accumulation, psqtaccumulation, color);
				}
				int16_t neweval = psqtaccumulation[WHITE][bucket] - psqtaccumulation[BLACK][bucket] - oldeval;
