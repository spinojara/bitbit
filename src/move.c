...
int pseudo_legal(const struct position *pos, const struct pstate *pstate, const move_t *move) {
	...
	case MOVE_CASTLE:
		...
		if (us) {
			 ...
			 * is faulty.
			 */
			if (pos->castle & 0x1 && to_square == g1 && !(all & 0x60) && !(pstate->attacked[ALL] & 0x60))
				return 1;
			if (pos->castle & 0x2 && to_square == c1 && !(all & 0xE) && !(pstate->attacked[ALL] & 0xC))
				return 1;
		}
		else {
			if (pos->castle & 0x4 && to_square == g8 && !(all & 0x6000000000000000)
			    && !(pstate->attacked[ALL] & 0x6000000000000000))
				return 1;
			if (pos->castle & 0x8 && to_square == c8 && !(all & 0xE00000000000000)
	...
	if (piece == PAWN) {
		...
		if ((from_square + pawn_sign == to_square && !pawn_push(from, all, us))
		    || (from_square + 2 * pawn_sign == to_square
		        && !(from & (us ? RANK_2 : RANK_7) && pawn_double_push(from, all, us)))
		    || (from_square + 1 + pawn_sign == to_square && !pawn_capture_e(from, pos->piece[them][ALL], us)
		        && (to_square != pos->en_passant || move_flag(move) != MOVE_EN_PASSANT))
		    || (from_square - 1 + pawn_sign == to_square && !pawn_capture_w(from, pos->piece[them][ALL], us)
		        && (to_square != pos->en_passant || move_flag(move) != MOVE_EN_PASSANT)))
			return 0;
	}
	...
	if (single(pstate->checkers)) {
		if (piece != KING
		    && !(to & pstate->checkray
		         || (piece == PAWN && pos->en_passant && to_square == pos->en_passant
...
char *move_str_pgn(char *str, const struct position *pos, const move_t *move) {
	...
	if (uncolored_piece(pos->mailbox[move_from(move)]) == PAWN) {
		...
		for (move_t *ptr = moves; *ptr; ptr++) {
			if (move_to(ptr) == move_to(move)
			    && pos->mailbox[move_from(ptr)] == pos->mailbox[move_from(move)])
